  ...
  show(note) {
    ...
    this.modalContent.innerHTML = `
      ...
      <div class="modal-action mt-6">
        <button class="btn btn-error" id="delete-btn" data-id="${note.id}">Delete</button>
        <button class="btn btn-outline" id="archive-btn" data-id="${note.id}" data-archived="${note.archived}">
    ...
    this.querySelector('#delete-btn').addEventListener('click', (e) => {
      this.dispatchEvent(new CustomEvent('delete-note', { detail: { id: e.target.dataset.id }, bubbles: true }));
      this.modal.close();
    });

    this.querySelector('#archive-btn').addEventListener('click', (e) => {
      const eventName = e.target.dataset.archived === 'true' ? 'unarchive-note' : 'archive-note';
      this.dispatchEvent(new CustomEvent(eventName, { detail: { id: e.target.dataset.id }, bubbles: true }));
      this.modal.close();
    });
  }
}
